...
import { constructPlayerHero, deleteByID, hydratePlayerHeroes, updateById, } from "../helpers/helpers.js";
...
router.put("/addItem/:userId/:itemId", async (req, res, next) => {
    try {
        ...
        const updated = await PlayerModel.findByIdAndUpdate(userId, {
...
router.put("/updateHero/:id", async (req, res, next) => {
    try {
        const { id } = req.params;
        const updatedHero = await updateById(id, "Hero", req.body, InputPlayerHero, PlayerHeroModel);
        logger.info(`Player Hero id: ${JSON.stringify(updatedHero?._id)} updated successfully`);
        res.status(200).send({
